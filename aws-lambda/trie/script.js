...
const apiUrl = __ENV.apiUrl || 'https://2x7bqep62m.execute-api.eu-west-1.amazonaws.com/prefix';
const testData = open("./data-60k.csv").split('\n');
const vus = __ENV.vus || 250;
const iterations = __ENV.iterations || 60;
...
function getPrice () {
  ...
  const called = item[1];
  const start = item[2];
  let res = http.get(`${apiUrl}?calling=${called}&date=${start}`);
  check(res, {
